...
The source workbook hash was unchanged and the populated project database was not opened by the independent tests. The subsequent DBeaver results confirmed the project table's schema, full month-end history and twelve PASS checks. The quarterly CPI modules, core analysis model and four Power BI pages have since been accepted. Continue through the [ABS rebuild guide](rebuild_abs_cpi.md), then the [SQL model](rebuild_analysis_model.md) and [Power BI](rebuild_power_bi.md) guides when reproducing the project.
